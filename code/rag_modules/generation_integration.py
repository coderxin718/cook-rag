...
请输出最终查询（如果不需要重写就返回原查询）:""",
    ...
    def intent_guard(self, query: str) -> bool:
        """
        意图守卫 - 判断用户问题是否与烹饪美食相关
...
你是一个意图分类器。判断用户问题是否与烹饪美食相关。

相关（YES）：菜品推荐、烹饪制作、食材询问、食谱浏览、厨房技巧
不相关（NO）：闲聊、编程、写作、数学、天气、新闻、娱乐、聊天等
...
用户问题: {query}""")
        ...
        result = chain.invoke(query).strip().upper()
...
1. 'recommend' - 用户想要获取菜品推荐或浏览菜品列表
   例如：推荐几个素菜、有什么川菜、今天吃什么、简单的汤有哪些

2. 'cook' - 用户想要具体的制作方法、步骤或食材信息
   例如：宫保鸡丁怎么做、需要什么食材、糖醋汁怎么调、炒菜技巧

只返回 recommend 或 cook
