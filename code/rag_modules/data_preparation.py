...
class DataPreparationModule:
    ...
    def _markdown_header_split(self) -> List[Document]:
        ...
        # 创建Markdown分割器
        # strip_headers=True: 标题存入metadata，不污染page_content（检索质量更好）
    ...
    def get_parent_documents(self, child_chunks: List[Document]) -> List[Document]:
        ...
        for chunk in child_chunks:
            # 优先使用 source 路径匹配（更稳定），回退到 parent_id 匹配
            source = chunk.metadata.get("source")
            parent_id = chunk.metadata.get("parent_id")

            # 尝试通过 source 路径找到父文档
            if source and source in source_to_doc:
                doc = source_to_doc[source]
                parent_id = doc.metadata.get("parent_id", parent_id)
            elif parent_id:
                # 回退：通过 parent_id 在 self.documents 中查找
                doc = None
                for d in self.documents:
                    if d.metadata.get("parent_id") == parent_id:
                        doc = d
                        break
            else:
                # 既没有 source 也没有 parent_id，跳过
                continue

            if doc is None:
                logger.debug(f"无法找到子块的父文档: dish={chunk.metadata.get('dish_name', '?')}, "
                           f"source={source}, parent_id={parent_id}")
                continue

            # 使用找到的父文档的 parent_id 作为去重键
            lookup_key = doc.metadata.get("parent_id", source)

            # 增加相关性计数
            parent_relevance[lookup_key] = parent_relevance.get(lookup_key, 0) + 1

            # 缓存父文档（避免重复查找）
            if lookup_key not in parent_docs_map:
                parent_docs_map[lookup_key] = doc
        ...
        for doc in parent_docs:
            ...
            lookup_key = doc.metadata.get('parent_id')
            relevance_count = parent_relevance.get(lookup_key, 0)
